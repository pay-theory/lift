...
| Quality | THR-7 | QUA-2 | Integration or contract tests stay green | Add integration/contract tests for public API behaviors (auth, routing, middleware ordering, tenant isolation semantics). | `./hgm-infra/verifiers/hgm-verify-rubric.sh` (QUA-2; runs `go test -tags=contract -count=1 ./pkg/contract/...`) | `hgm-infra/evidence/QUA-2-output.log` |
...
| Consistency | THR-1, THR-4 | CON-3 | Public boundary contract parity (if applicable) | Exported/public API behaviors are backed by deterministic contract tests to prevent accidental semantic drift. | `./hgm-infra/verifiers/hgm-verify-rubric.sh` (CON-3; runs `go test -tags=contract -count=1 ./pkg/contract/...`) | `hgm-infra/evidence/CON-3-output.log` |
...
| Completeness | THR-2 | COM-6 | Logging/operational standards enforced | Prevent logging of secrets/CHD-adjacent payloads; require structured logs; enforce correlation IDs. | `./hgm-infra/verifiers/hgm-verify-rubric.sh` (COM-6; runs `go test -count=1 -run '^TestOps_' ./pkg/observability/zap ./pkg/middleware`) | `hgm-infra/evidence/COM-6-output.log` |
| Completeness | THR-7 | COM-7 | CI enforces rubric surface | CI runs the full rubric surface as a single command on PRs/pushes to protected branches, and uploads evidence artifacts. | verifier COM-7 | `hgm-infra/evidence/COM-7-output.log` |
| Completeness | THR-5, THR-7 | COM-8 | Branch/release automation enforced | Branch + release policy + workflows are defined (main releases, premain prereleases) and security workflows run on both branches. | verifier COM-8 | `hgm-infra/evidence/COM-8-output.log` |
...
| Security | THR-5 | SEC-2 | Dependency vulnerability scan stays green | govulncheck (pinned) is run and results are reviewed; CI blocks on findings. | verifier SEC-2 | `hgm-infra/evidence/SEC-2-output.log` |
...
| Security | THR-1, THR-2, THR-3, THR-4, THR-6, THR-8, THR-9 | SEC-4 | Domain-specific P0 regression tests | Add P0 tests asserting critical security invariants (no auth bypass, safe JWT handling, request size guardrails, safe logging defaults, secrets handling). | `./hgm-infra/verifiers/hgm-verify-rubric.sh` (SEC-4; runs `go test -count=1 -run '^TestP0_' ./pkg/observability/zap ./pkg/middleware`) | `hgm-infra/evidence/SEC-4-output.log` |
