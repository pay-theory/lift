...
- Coverage: `make rubric` (QUA-3/COM-4) →
...
- Branch/release policy: `hgm-infra/planning/lift-branch-release-policy.md`
...
| --- | --- | --- | --- |
| QUA-1 | Unit test output | `hgm-infra/evidence/QUA-1-output.log` | `make rubric` |
| QUA-2 | Integration/contract output | `hgm-infra/evidence/QUA-2-output.log` | `make rubric` (runs `go test -tags=contract -count=1 ./pkg/contract/...`) |
| QUA-3 | Coverage profile + summary | `hgm-infra/evidence/QUA-3-output.log` | `make rubric` |
| CON-1 | Formatter diff list | `hgm-infra/evidence/CON-1-output.log` | `make rubric` |
| CON-2 | Lint output | `hgm-infra/evidence/CON-2-output.log` | `make rubric` |
| CON-3 | Contract verification output | `hgm-infra/evidence/CON-3-output.log` | `make rubric` (runs `go test -tags=contract -count=1 ./pkg/contract/...`) |
| COM-1 | Module compile check | `hgm-infra/evidence/COM-1-output.log` | `make rubric` |
| COM-2 | Toolchain pin verification | `hgm-infra/evidence/COM-2-output.log` | `make rubric` |
| COM-3 | Lint config validation | `hgm-infra/evidence/COM-3-output.log` | `make rubric` |
| COM-4 | Coverage threshold check | `hgm-infra/evidence/COM-4-output.log` | `make rubric` |
| COM-5 | Security config validation | `hgm-infra/evidence/COM-5-output.log` | `make rubric` |
| COM-6 | Logging standards check | `hgm-infra/evidence/COM-6-output.log` | `make rubric` (runs `go test -count=1 -run '^TestOps_' ./pkg/observability/zap ./pkg/middleware`) |
| COM-7 | CI rubric enforcement check | `hgm-infra/evidence/COM-7-output.log` | `make rubric` |
| COM-8 | Branch/release supply-chain check | `hgm-infra/evidence/COM-8-output.log` | `make rubric` |
| SEC-1 | SAST scan output | `hgm-infra/evidence/SEC-1-output.log` | `make rubric` |
| SEC-2 | Vulnerability scan output | `hgm-infra/evidence/SEC-2-output.log` | `make rubric` |
| SEC-3 | Supply-chain verification | `hgm-infra/evidence/SEC-3-output.log` | `make rubric` |
| SEC-4 | Domain P0 regression tests | `hgm-infra/evidence/SEC-4-output.log` | `make rubric` (runs `go test -count=1 -run '^TestP0_' ./pkg/observability/zap ./pkg/middleware`) |
...
| MAI-1 | File budget check | `hgm-infra/evidence/MAI-1-output.log` | `make rubric` |
| MAI-2 | Maintainability roadmap check | `hgm-infra/evidence/MAI-2-output.log` | `make rubric` |
| MAI-3 | Duplicate semantics check | `hgm-infra/evidence/MAI-3-output.log` | `make rubric` |
...
| DOC-4 | Doc integrity output | `hgm-infra/evidence/DOC-4-output.log` | `make rubric` |
| DOC-5 | Threat ↔ controls parity | `hgm-infra/evidence/DOC-5-parity.log` | `make rubric` |
